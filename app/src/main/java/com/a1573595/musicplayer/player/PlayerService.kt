    ...
    override fun update(o: Observable?, any: Any?) {
        when (any) {
            ACTION_COMPLETE -> {
                ...
                when {
                    isRepeat -> play()
                    isRandom -> play((0 until musicList.size).random())
                    else -> skipToNext()
                }
            }
...
//                stopForeground(true)
    ...
    suspend fun readSong() = withContext(Dispatchers.IO) {
        if (musicList.isNotEmpty()) return@withContext
    ...
    fun getSongList() = musicList.toList()

    fun getSong(): Music? {
        return if (musicList.size > 0) {
            musicList[playerPosition]
        } else {
            null
        }
    }
    ...
    fun play(position: Int = playerPosition) {
        ...
        playerPosition = when {
            ...
            position >= musicList.size -> 0
            position < 0 -> musicList.lastIndex
            else -> position
        }

        val audioUri = Uri.withAppendedPath(uriExternal, musicList[playerPosition].id)
        ...
        try {
            ...
            musicList.removeAt(playerPosition)
            playerManager.setChangedNotify(ACTION_NOT_SONG_FOUND)
    ...
    fun skipToNext() {
        if (!isRandom) {
            ...
            play((0 until musicList.size).random())
        }
    }
    ...
    private fun createNotification(): Notification {
        ...
        smallRemoteView.setTextViewText(R.id.tv_name, song?.title)
        smallRemoteView.setImageViewResource(
            R.id.img_play,
            if (isPlaying) R.drawable.ic_pause else R.drawable.ic_play
        )

        largeRemoteView.setTextViewText(R.id.tv_name, song?.title)
